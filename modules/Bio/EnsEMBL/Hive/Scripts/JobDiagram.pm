...
package Bio::EnsEMBL::Hive::Scripts::JobDiagram;
...
sub command_line_options {
    return [
        ...
        'job_id=s@'             => 'job_ids',                 # jobs to start from
        'start_analysis_name=s' => 'start_analysis_name',     # if given, first trace the graph up to the given analysis or the seed_jobs, and then start visualization
        'stop_analysis_name=s'  => 'stop_analysis_name',      # if given, the visualization is aborted at that analysis and doesn't go any further

        'include!'              => 'include',                 # if set, include other pipeline rectangles inside the main one
        'suppress_funnel_parent_link!'  => 'suppress',        # if set, do not show the link to the parent of a funnel job (potentially less clutter)

        'accu_keys|accus!'      => 'show_accu_keys',          # show accu keys, but not necessarily values
        'accu_values|values!'   => 'show_accu_values',        # show accu keys & values (implies -accu_keys)
        'accu_pointers|accu_ptrs!' => 'show_accu_pointers',   # (attempt to) show which accu values come from which jobs

        'o|out|output=s'        => 'output',                  # output file name
        'f|format=s'            => 'format',                  # output format (if not guessable from -output)
...
sub run {
    ...
    if($self->{'output'}) {
        ...
        precache_participating_pipelines( $self->param('pipeline') );

        my $job_adaptor     = $self->param('pipeline')->hive_dba->get_AnalysisJobAdaptor;
        my $anchor_jobs     = $self->{'job_ids'} && $job_adaptor->fetch_all( 'job_id IN ('.join(',', @{$self->{'job_ids'}} ).')' );
        my $start_analysis  = $self->param('start_analysis_name') && $self->param('pipeline')->find_by_query( {'object_type' => 'Analysis', 'logic_name' => $self->param('start_analysis_name') } );
        my $stop_analysis   = $self->param('stop_analysis_name') && $self->param('pipeline')->find_by_query( {'object_type' => 'Analysis', 'logic_name' => $self->param('stop_analysis_name') } );

        $self->param('stop_analysis', $stop_analysis);

        $self->param('semaphore_url_hash', {});
        $self->param('analysis_name_2_pipeline', {});
...
if(0) {
        for (1..2) {    # a hacky way to get relative independence on sorting order (we don't know the ideal sorting order)
            foreach my $semaphore_url ( keys %{$self->param('semaphore_url_hash')} ) {
                foreach my $remote_semaphore ( @{ Bio::EnsEMBL::Hive::TheApiary->fetch_remote_semaphores_controlling_this_one( $semaphore_url, $self->param('pipeline') ) } ) {
        ...
        foreach my $analysis_name (keys %{$self->param('analysis_name_2_pipeline')}) {
            my $this_pipeline = $self->param('analysis_name_2_pipeline')->{$analysis_name};
            push @{ $self->{'graph'}->cluster_2_nodes->{ $this_pipeline->hive_pipeline_name } }, $analysis_name;
        }

        $self->{'graph'}->cluster_2_attributes->{ $self->param('pipeline')->hive_pipeline_name }{ 'display_cluster_name' } = 1;
        $self->{'graph'}->cluster_2_attributes->{ $self->param('pipeline')->hive_pipeline_name }{ 'style' } = 'bold,filled';
        $self->{'graph'}->cluster_2_attributes->{ $self->param('pipeline')->hive_pipeline_name }{ 'fill_colour_pair' } = ['pastel19', 3];
        ...
        foreach my $other_pipeline ( @{ Bio::EnsEMBL::Hive::TheApiary->pipelines_except($self->param('pipeline')) } ) {
            ...
            if($self->{'include'}) {
                push @{ $self->{'graph'}->cluster_2_nodes->{ $self->param('pipeline')->hive_pipeline_name } }, $other_pipeline->hive_pipeline_name;
            }
        }
...
sub trace_semaphore_up {
    ...
    foreach my $remote_semaphore ( @{ Bio::EnsEMBL::Hive::TheApiary->fetch_remote_semaphores_controlling_this_one( $semaphore, $self->param('pipeline') ) } ) {
...
sub draw_job_node {
    ...
    my $analysis_name   = $job->analysis->relative_display_name($self->param('pipeline'));
    $analysis_name=~s{/}{___};
    ...
    $self->parma('analysis_name_2_pipeline')->{ $analysis_name } = $job->hive_pipeline;
}
...
sub add_job_node {
    ...
    unless($job_node_hash{$job_node_name}++) {
        ...
        if( !$self->param('stop_analysis') or ($job->analysis != $self->param('stop_analysis')) ) {
...
sub draw_semaphore_and_accu {
    my ($self, $semaphore, $dependent_node_name) = @_;
...
sub add_semaphore_node {
    ...
    unless($self->parma('semaphore_url_hash')->{$semaphore_url}++) {
        ...
        if(my $dependent_job = $semaphore->dependent_job) {
            ...
            $target_cluster_name = $dependent_job->analysis->relative_display_name($self->param('pipeline'));

            if($dependent_job->analysis->hive_pipeline->hive_pipeline_name eq $self->param('pipeline')->hive_pipeline_name) {
                $target_cluster_name =~s{^.*/}{};   # workaround since we may have added $self->param('pipeline') into TheApiary
            } else {
