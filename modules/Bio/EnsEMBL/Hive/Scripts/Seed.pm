...
package Bio::EnsEMBL::Hive::Scripts::Seed;
...
sub run {
    ...
    my $analysis;
    if(my $analyses_pattern = $self->param('analyses_pattern') || $self->param('analysis_id') || $self->param('logic_name')) {

        my $candidate_analyses = $self->param('pipeline')->collection_of( 'Analysis' )->find_all_by_pattern( $analyses_pattern );
        ...
        show_seedable_analyses($self->param('pipeline'));
        return;
    }
    ...
    my ($job_id) = @{ $self->param('pipeline')->hive_dba->get_AnalysisJobAdaptor->store_jobs_and_adjust_counters( [ $job ] ) };
