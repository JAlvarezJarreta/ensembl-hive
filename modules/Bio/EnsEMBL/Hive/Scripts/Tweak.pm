...
package Bio::EnsEMBL::Hive::Scripts::Tweak;
...
sub run {
    ...
    if(@{$self->param('tweaks')}) {
        my $need_write = $self->param('pipeline')->apply_tweaks( $self->param('tweaks') );
        if ($need_write) {
            $self->param('pipeline')->save_collections();
        }
    }
