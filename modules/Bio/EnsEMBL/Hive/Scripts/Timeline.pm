...
package Bio::EnsEMBL::Hive::Scripts::Timeline;
...
sub command_line_options {
    return [
            ...
            'key=s'                      => ['key', 'analysis'],
            'top=f'                      => 'top',
            'mem=i'                      => ['default_memory', 100],    # Default value is 100Mb
            'n_core=i'                   => ['default_cores', 1],       # Default value is 1 core
            'output=s'                   => 'output',
    ];
}
...
sub run {
    ...
    if ($self->param('output') and $self->param('output') =~ /\.(\w+)$/) {
        $gnuplot_terminal = $1;
    ...
    my $pipeline = $self->param('pipeline');
    ...
    my %layers = ();
    {
        ...
        foreach my $db_entry (@tmp_dates) {
            ...
            if ($mode eq 'workers') {
                ...
                my $offset = ($mem_resources{$resource_class_id} || $self->param('default_memory')) / 1024.;
                ...
                my $offset = ($cpu_resources{$resource_class_id} || $self->param('default_cores'));
                add_event(\%events, $key_value, $when_born, $when_died, $offset);
    ...
    my ($n_relevant_analysis, $need_other_analysis, $real_top) = count_number_relevant_sets(\@sorted_key_ids, \%tot_analysis, $total_total, $self->param('top'), scalar(@palette), $verbose);
